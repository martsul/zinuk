export class ControlService {
  ...
  #nextPage() {
    if (!(!this.#nextPageLink || (this.#answerField && !this.#answerField.value))) {
  ...
  #scrollToActiveParagraph() {
    const activeParagraph = this.#questionsParagraphs[this.#activeParagraph - 1];
    activeParagraph.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }

  #changeActiveAudio() {
    this.#activeAudio = new Audio(this.#questionsParagraphs[this.#activeParagraph - 1].dataset.audio)
  }
